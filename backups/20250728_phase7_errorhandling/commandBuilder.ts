        ...
        const disposable = vscode.commands.registerCommand(
            ...
            async (...args: any[]) => {
                ...
                try {
                    ...
                    console.error(`${config.module}: Error in ${config.description}:`, error);

                    if (!config.silentErrors) {
                        const prefix = config.errorMessage ?? `Failed: ${config.description}`;
                        vscode.window.showErrorMessage(`${prefix}: ${errorStr}`);
                    }
                }
            },
        );
