...
- `codexr/<component>/` — per-feature runtimes: pointer-policy (single-active-pointer arbitration: mouse / gaze / laser, following the last-used controller), immersive-rig (turns `movement-controls` fly on for the duration of any immersive session, and recenters the rig next to the pedestal in AR only; never touches the rig's y — eye height lives permanently on the rig position in the scene template, the one transform WebXR never rewrites), analysis-table, chart containment, mapping UI, dependency-graph, historical-comparison, project-evolution, virtual-screen, collaboration, avatar, xr-room, render budget, debug. VR/AR locomotion is deliberately NOT a CodeXR component: it is native aframe-extras `movement-controls`/`gamepad-controls` (left stick moves, right stick turns), which requires `gamepad` to stay in the default `controls` list.
