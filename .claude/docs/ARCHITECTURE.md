...
| `servers/` | Server **infrastructure**: HTTP/HTTPS, ports, self-signed certs, SSE, collaboration/broadcast signaling | `runtime/httpServer.ts` (façade + composition root), `runtime/sse/SSEManager.ts`, `runtime/collaboration/collaborationRoomServer.ts` |
| `servers/runtime/http/` | Response helpers (`httpRespond.ts`: free functions `sendJsonResponse`/`sendErrorResponse`/CORS/body reading) + `StaticAssetServer` (pages, MIME, traversal guard, no-store) | `httpRespond.ts`, `staticAssets.ts` |
| `servers/runtime/remote/` | Remote pairing + authorization: `RemoteAccessPolicy` (request/WS auth, cookie-free allowlist) and `RemotePairingApi` (identity → pair → confirm → browser token exchange, guest page) | `remoteAccessPolicy.ts`, `remotePairingApi.ts`, `pairingPage.ts` |
| `servers/runtime/collaboration/` | WebSocket room + session descriptor API (`CollaborationSessionApi`: descriptor, avatar model, room id) | `collaborationRoomServer.ts`, `collaborationSessionApi.ts` |
| `servers/runtime/analysis/` | Analysis features behind the server: `AnalysisFeatureHost` (capability-gated service construction, refresh wiring, LivePanel SSE notifiers, dispose) + one bridge per feature + the room-message router. Bridges depend on the host; nothing here imports the façade | `analysisFeatureHost.ts`, `analysisMessageRouter.ts`, `dependencyGraphBridge.ts`, `historicalComparisonBridge.ts`, `projectEvolutionBridge.ts` |
