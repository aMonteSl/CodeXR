...
def main():
    ...
    try:
        if args.mode == 'schema':
            log_debug('Returning XR schema metadata...')
            result = execute_schema_request(args.type)
        elif args.mode == 'livePanel' and args.type == 'file':
...
def execute_livepanel_file_analysis(target_file):
    ...
    try:
        ...
        return analyze_file_comprehensive(target_file)
...
def execute_schema_request(target_type):
    """Return the XR field schema used by the mapping UI."""
