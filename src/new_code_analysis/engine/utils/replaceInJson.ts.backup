...
export class ReplaceInJson {
     ...
     * @param sessionId Session ID for saveDataJsonWithOverride
     * @param context VS Code extension context for saveDataJsonWithOverride
    ...
    static async replaceFileAnalysisInJson(
        ...
        try {
            ...
            let analysisData = JSON.parse(jsonContent);

            // Detect format: XR uses direct array, LivePanel uses object with files property
            let isXRFormat = Array.isArray(analysisData);
            let filesArray: any[];
            
            if (isXRFormat) {
                // XR format: JSON is a direct array of files
                filesArray = analysisData;
                console.log(`REPLACE_JSON_D_LIVE_PANEL: Detected XR format (direct array)`);
                console.log(`REPLACE_JSON_D_LIVE_PANEL: Existing files array length: ${filesArray.length}`);
                
                // Create a temporary object structure for processing
                analysisData = {
                    files: filesArray,
                    summary: {} // XR doesn't use summary but we'll maintain consistency
                };
            } else {
                // LivePanel format: JSON is an object with files property
                filesArray = analysisData.files || [];
                console.log(`REPLACE_JSON_D_LIVE_PANEL: Detected LivePanel format (object with files property)`);
                console.log(`REPLACE_JSON_D_LIVE_PANEL: Existing files array length: ${filesArray.length}`);
            ...
            // Write updated JSON back to file - maintain original format
            let finalData: any;
            if (isXRFormat) {
                // For XR format: save as direct array (just the files)
                finalData = analysisData.files;
                console.log(`REPLACE_JSON_D_LIVE_PANEL: Saving in XR format (direct array) with ${finalData.length} files`);
            } else {
                // For LivePanel format: save as object with files property and summary
                finalData = analysisData;
                console.log(`REPLACE_JSON_D_LIVE_PANEL: Saving in LivePanel format (object) with ${analysisData.files.length} files`);
            }
            
            // Use saveDataJsonWithOverride if sessionId and context are provided (better for SSE)
            if (sessionId && context) {
                console.log(`REPLACE_JSON_D_LIVE_PANEL: Using saveDataJsonWithOverride for better SSE support`);
                const saveResult = await SaveFiles.saveDataJsonWithOverride(sessionId, finalData, context);
                
                if (!saveResult.success) {
                    throw new Error(`Failed to save with override: ${saveResult.error}`);
                }
                
                console.log(`REPLACE_JSON_D_LIVE_PANEL: ✅ Successfully saved using saveDataJsonWithOverride`);
            } else {
                // Fallback to direct file write (legacy method)
                console.log(`REPLACE_JSON_D_LIVE_PANEL: Using direct file write (legacy method)`);
                const updatedJsonContent = JSON.stringify(finalData, null, 2);
    ...
    /**
     * Common function to safely read and parse JSON file
     */
    private static readAnalysisJson(analysisJsonPath: string): any {
        if (!fs.existsSync(analysisJsonPath)) {
            throw new Error(`Analysis JSON file not found: ${analysisJsonPath}`);
        }
    ...
    /**
     * Common function to safely write JSON file
     */
    private static writeAnalysisJson(analysisJsonPath: string, data: any): void {
        const jsonContent = JSON.stringify(data, null, 2);
        fs.writeFileSync(analysisJsonPath, jsonContent, 'utf-8');
    }
}
