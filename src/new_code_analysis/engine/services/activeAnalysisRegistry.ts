...
export class ActiveAnalysisRegistry {
    ...
    public async registerFileLivePanelAnalysis(
        ...
        try {
            ...
            // Create session in file registry
            const session = await this.fileSessionRegistry.createSession(filePath, 'LivePanel', context);
    ...
    public async registerDirectoryLivePanelAnalysis(
        ...
        try {
            console.log(`ACTIVE_REGISTRY_DEBUG: 📁 Starting Directory LivePanel registration for: ${directoryPath}`);
            ...
            const session: AnalysisSession = {
                ...
                outputDirectory: `${dirName}_DirectoryLivePanel_${id}`,
                outputPath: path.join(context.storageUri?.fsPath || '/tmp', 'analysis', `${dirName}_DirectoryLivePanel_${id}`),
            ...
            console.log(`ACTIVE_REGISTRY_DEBUG: ✅ Directory session created with ID: ${session.id}`);
            console.log(`ACTIVE_REGISTRY_DEBUG: 📋 Session details: type=${session.analysisType}, status=${session.status}`);
            ...
            console.log(`ACTIVE_REGISTRY_DEBUG: ✅ Directory LivePanel analysis registered and UI refreshed with session ID: ${session.id}`);
            return session.id;
