...
export class SaveFiles {
    ...
    static async saveAnalysisFiles(
        ...
        try {
            ...
            // Always copy favicon.ico from resources
            await SaveFiles.copyFavicon(analysisSessionDir);
            ...
            console.log(`SAVE_FILES: - favicon.ico: ${path.join(analysisSessionDir, 'favicon.ico')}`);
    ...
    /**
     * Clean the complete workspace storage (called on plugin restart)
     * This removes everything in the workspace storage directory
     */
    static async cleanCompleteWorkspaceStorage(context: vscode.ExtensionContext): Promise<boolean> {
        try {
            console.log(`SAVE_FILES: Starting complete workspace storage cleanup`);
            ...
            const workspaceStorageDir = storageUri.fsPath;
            console.log(`SAVE_FILES: Cleaning complete workspace storage at: ${workspaceStorageDir}`);

            if (fs.existsSync(workspaceStorageDir)) {
                // Get all entries in the workspace storage directory
                const allEntries = await fs.promises.readdir(workspaceStorageDir, { withFileTypes: true });
                console.log(`SAVE_FILES: Found ${allEntries.length} entries to clean in workspace storage`);

                let success = true;

                // Remove all files and directories
                for (const entry of allEntries) {
                    const entryPath = path.join(workspaceStorageDir, entry.name);
                    try {
                        if (entry.isDirectory()) {
                            await fs.promises.rm(entryPath, { recursive: true, force: true });
                            console.log(`SAVE_FILES: Cleaned directory: ${entry.name}`);
                        } else {
                            await fs.promises.unlink(entryPath);
                            console.log(`SAVE_FILES: Cleaned file: ${entry.name}`);
                        }
                    } catch (entryError) {
                        console.error(`SAVE_FILES: Failed to clean entry ${entry.name}:`, entryError);
                        success = false;
                    }
                }

                console.log(`SAVE_FILES: Complete workspace storage cleanup completed`);
                return success;
            } else {
                console.log(`SAVE_FILES: Workspace storage directory doesn't exist - nothing to clean`);
                return true;
            }

        } catch (error) {
            console.error(`SAVE_FILES: Error during complete workspace storage cleanup:`, error);
            return false;
        }
    }
    ...
    /**
     * Copy favicon.ico from resources to the specified directory
     * 
     * @param targetDirectory - Directory where favicon should be copied
     */
    static async copyFavicon(targetDirectory: string): Promise<void> {
        try {
            // Get extension context from vscode to access extension resources
            const extensionPath = vscode.extensions.getExtension('aMonteSl.code-xr')?.extensionPath;
            if (!extensionPath) {
                console.warn(`SAVE_FILES: ⚠️ Extension path not found, skipping favicon copy`);
                return;
            }
            
            // Build path to favicon in extension resources
            const faviconSourcePath = path.join(extensionPath, 'resources', 'favicon.ico');
            const faviconDestPath = path.join(targetDirectory, 'favicon.ico');
            ...
            // Don't throw error to avoid interrupting other file saves
        }
    }
}
