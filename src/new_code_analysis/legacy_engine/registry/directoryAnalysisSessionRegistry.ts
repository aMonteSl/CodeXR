...
export class DirectoryAnalysisSessionRegistry {
    ...
    registerSessionPort(sessionId: string, port: number): boolean {
        console.log(`DIRECTORY_ANALYSIS_REGISTRY: 🔍 DEBUG - Registering port ${port} for session ${sessionId}`);
        
        const session = this.sessions.get(sessionId);
        if (!session) {
            console.warn(`DIRECTORY_ANALYSIS_REGISTRY: ❌ Session ${sessionId} not found for port registration`);
            return false;
        }

        console.log(`DIRECTORY_ANALYSIS_REGISTRY: 🔍 DEBUG - Session before port assignment:`, {
            id: session.id,
            nameDir: session.nameDir,
        ...
        console.log(`DIRECTORY_ANALYSIS_REGISTRY: ✅ Successfully registered port ${port} for session ${sessionId}`);
        console.log(`DIRECTORY_ANALYSIS_REGISTRY: 🔍 DEBUG - Session after port assignment:`, {
            id: session.id,
            nameDir: session.nameDir,
