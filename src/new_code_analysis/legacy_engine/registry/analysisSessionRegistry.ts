...
export type AnalysisType = 'FileLivePanel' | 'DOMVisualization' | 'FileXRAnalysis' | 'DirectoryLivePanel' | 'DeepDirectoryLivePanel' | 'DirectoryXRAnalysis' | 'DeepDirectoryXRAnalysis';
...
export class AnalysisSessionRegistry {
    ...
    registerSessionPort(sessionId: string, port: number): boolean {
        console.log(`ANALYSIS_REGISTRY: 🔍 DEBUG - Registering port ${port} for session ${sessionId}`);
        
        const session = this.sessions.get(sessionId);
        if (!session) {
            console.warn(`ANALYSIS_REGISTRY: ❌ Session ${sessionId} not found for port registration`);
            return false;
        }

        console.log(`ANALYSIS_REGISTRY: 🔍 DEBUG - Session before port assignment:`, {
            id: session.id,
        ...
        console.log(`ANALYSIS_REGISTRY: ✅ Successfully registered port ${port} for session ${sessionId}`);
        console.log(`ANALYSIS_REGISTRY: 🔍 DEBUG - Session after port assignment:`, {
            id: session.id,
            filePath: session.filePath,
            assignedPort: session.port,
            status: session.status
        });
    ...
    getStatistics(): {
        ...
        const stats = {
            ...
            byType: {
                'FileLivePanel': sessions.filter(s => s.analysisType === 'FileLivePanel').length,
                'DirectoryLivePanel': sessions.filter(s => s.analysisType === 'DirectoryLivePanel').length,
                'DeepDirectoryLivePanel': sessions.filter(s => s.analysisType === 'DeepDirectoryLivePanel').length,
                ...
                'DirectoryXRAnalysis': sessions.filter(s => s.analysisType === 'DirectoryXRAnalysis').length,
                'DeepDirectoryXRAnalysis': sessions.filter(s => s.analysisType === 'DeepDirectoryXRAnalysis').length
