...
def analyze_comments(file_path):
    ...
    elif ext in ['.js', '.ts', '.jsx', '.tsx', '.c', '.cpp', '.cc', '.cxx', '.h', '.hpp', '.cs', '.java', '.sol', '.m', '.zig', '.ttcn', '.ttcn3', '.go', '.kt', '.kts', '.php', '.scala', '.sc', '.swift']:
    ...
    elif ext in ['.pl', '.pm', '.pod', '.t']:  # Perl
        comment_lines = analyze_hash_comments(content)
    elif ext in ['.lua']:  # Lua
        comment_lines = analyze_lua_comments(content)
    elif ext in ['.erl', '.hrl']:  # Erlang
...
def analyze_erlang_comments(content):
    ...
    for line_num, line in enumerate(lines, 1):
        # Find comment marker, but ignore if inside string
        in_string = False
        escape_next = False
