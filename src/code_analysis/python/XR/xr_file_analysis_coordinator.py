...
from xr_field_schema import build_complexity_band
    ...
    try:
        ...
        for func in lizard_data.get('functions', []):
            ...
            xr_function = {
                ...
                "spanLines": max(0, (func.get('lineEnd', 0) - func.get('lineStart', 0)) + 1),
                "complexity": func.get('complexity', 1), # Fixed: use correct field name
                "complexityBand": build_complexity_band(func.get('complexity', 1)),
                ...
                "filePath": file_path,
                "fileName": file_name,
                "treePath": build_tree_path(file_name, function_name)
            }
        ...
        return normalize_output_paths(xr_functions)
...
def generate_fallback_xr_data(file_path):
    ...
    fallback_function = {
        ...
        "spanLines": total_lines,
        "complexity": 1,
        "complexityBand": build_complexity_band(1),
        "parameters": 0,
        "maxNestingDepth": 0,
        ...
        "filePath": file_path,
        "fileName": file_name,
        "treePath": build_tree_path(file_name, base_name)
    }
    
    print(json.dumps({"debug": f"XR_ANALYSIS: Generated fallback data for {file_name}"}), file=sys.stderr)
    
    return normalize_output_paths([fallback_function])
