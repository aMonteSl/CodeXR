...
export class SessionWatcherManager {
    ...
    async startWatchingSession(session: UnifiedAnalysisSession): Promise<string | null> {
        try {
            ...
            if (session.analysisMode !== 'LivePanel') {
                console.log(`SESSION_WATCHER_MANAGER:  Skipping watcher for ${session.analysisMode} (not implemented yet)`);
                return null;
            }
    ...
    async stopWatchingSession(sessionId: string): Promise<boolean> {
        try {
            ...
            if (!orchestrator) {
                console.log(`SESSION_WATCHER_MANAGER:  No active watcher found for session ${sessionId}`);
                return false;
            }
