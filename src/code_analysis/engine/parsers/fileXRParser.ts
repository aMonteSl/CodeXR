...
export class FileXRParser {
    ...
    public async parseFileAnalysis(session: UnifiedAnalysisSession, theme?: string): Promise<ParsedXRFileAnalysis> {
        ...
        try {
            ...
            if (validationResult.warnings.length > 0) {
                console.log(`FILE_XR_PARSER:  Validation warnings:`, validationResult.warnings);
            }
            ...
            const dataSource = 'data.json'; // Match XR directory analysis data source
            ...
            if (loadedFiles.size > 0) {
                ...
                expectedFiles.forEach(expectedFile => {
                    if (loadedFiles.has(expectedFile)) {
                        ...
                        console.log(`FILE_XR_PARSER:   Expected file missing: ${expectedFile}`);
                    }
                });
            } else {
                console.log(`FILE_XR_PARSER:   No files were loaded from output directory`);
            }
    ...
    private displayConfigurationInfo(
        ...
        console.log(` Chart Category: ${chartMetadata.category}`);
        console.log(` Total Dimensions: ${chartMetadata.dimensions.length}`);

        console.log(`\n === DIMENSION MAPPINGS ===`);
        if (dimensionMappings.length === 0) {
            console.log(` No dimension mappings configured!`);
        } else {
