...
import { CodeAnalysisInteractionHandler } from '../views/interactions/handleAnalysisClicks';
import { AnalysisSettingsStorage } from '../../utils/analysisSettingsStorage';
import { PythonEnvStorage } from '../../python_env/storage/pythonEnvStorage';
import { PythonEnvUtils } from '../../python_env/utils/pythonEnvUtils';
import { storeAnalysisJson, prepareStaticAnalysisViewerAssets, prepareXRAnalysisViewerAssets } from '../utils/tempStorageManager';
import { ActiveAnalysisRegistry } from '../active_analyses/registry/activeAnalysisRegistry';
import { ActiveAnalysisFactory } from '../active_analyses/model/activeAnalysisModel';
import { BabiaChartRegistry } from '../../babia_templates/registry/chartRegistry';
import { XRTemplateRenderer } from '../utils/xrTemplateRenderer';
...
export class CodeAnalysisCommands {
    ...
    static registerCommands(context: vscode.ExtensionContext): void {
        ...
        const showActiveAnalysesCommand = vscode.commands.registerCommand(
            'codeXR.codeAnalysis.showActiveAnalyses',
            () => {
                console.log('[CODE_ANALYSIS] Command: showActiveAnalyses executed');
                CodeAnalysisInteractionHandler.handleSectionClick('active-analyses', context);
            }
        );
        
        const showAnalysisSettingsCommand = vscode.commands.registerCommand(
            'codeXR.codeAnalysis.showAnalysisSettings',
            () => {
                console.log('[CODE_ANALYSIS] Command: showAnalysisSettings executed');
                CodeAnalysisInteractionHandler.handleSectionClick('analysis-settings', context);
            }
        );
        
        const showFilesByLanguageCommand = vscode.commands.registerCommand(
            'codeXR.codeAnalysis.showFilesByLanguage',
            () => {
                console.log('[CODE_ANALYSIS] Command: showFilesByLanguage executed');
                CodeAnalysisInteractionHandler.handleSectionClick('files-by-language', context);
            }
        );
        
        // Placeholder commands
        const placeholderActiveAnalysesCommand = vscode.commands.registerCommand(
            'codeXR.codeAnalysis.placeholder.activeAnalyses',
            () => {
                console.log('[CODE_ANALYSIS] Command: placeholder.activeAnalyses executed');
                CodeAnalysisInteractionHandler.handlePlaceholderClick('activeAnalyses');
            }
        );
        
        const placeholderAnalysisSettingsCommand = vscode.commands.registerCommand(
            'codeXR.codeAnalysis.placeholder.analysisSettings',
            () => {
                console.log('[CODE_ANALYSIS] Command: placeholder.analysisSettings executed');
                CodeAnalysisInteractionHandler.handlePlaceholderClick('analysisSettings');
            }
        );
        
        const placeholderFilesByLanguageCommand = vscode.commands.registerCommand(
            'codeXR.codeAnalysis.placeholder.filesByLanguage',
            () => {
                console.log('[CODE_ANALYSIS] Command: placeholder.filesByLanguage executed');
                CodeAnalysisInteractionHandler.handlePlaceholderClick('filesByLanguage');
            }
        );
        
        // Analysis mode toggle command
        const toggleAnalysisModeCommand = vscode.commands.registerCommand(
            'codexr.analysis.toggleMode',
            async () => {
                console.log('[CODE_ANALYSIS] Command: toggleMode executed');
                try {
                    const newMode = await AnalysisSettingsStorage.toggleAnalysisMode(context);
        ...
        const analyzeFileStaticCommand = vscode.commands.registerCommand(
            'codexr.analysis.fileStatic',
            async (uri?: vscode.Uri) => {
                ...
                try {
                    ...
                    const registry = ActiveAnalysisRegistry.getInstance();
                    {
                        const analysisTemplate = ActiveAnalysisFactory.createFileAnalysis(
                            actualFilePath,
                            'Static',
                            path.extname(actualFilePath).toLowerCase().substring(1) || 'unknown'
                        );
                    ...
                    if (analysisData) {
                        ...
                        if (currentMode === 'Static') {
                            ...
                            try {
                                ...
                                await prepareStaticAnalysisViewerAssets(context, targetUri.fsPath, analysisData);
                                ...
                                try {
                                    const storageUri = await storeAnalysisJson(context, targetUri.fsPath, analysisData);
                        ...
                        vscode.window.showInformationMessage(
                            `Analysis completed: ${functionCount} functions, ${classCount} classes, ` +
            ...
            async (uri?: vscode.Uri) => {
                ...
                try {
                    ...
                    if (analysisData) {
                        ...
                        try {
                            const tempFolder = await storeAnalysisJson(context, actualFilePath, analysisData);
                            ...
                            try {
                                ...
                                await XRTemplateRenderer.generateXRVisualization(context, tempFolder, actualFilePath, analysisData);
                                ...
                                try {
                                    ...
                                    await prepareXRAnalysisViewerAssets(context, tempFolder, actualFilePath);
        ...
        // File click command
        const fileClickedCommand = vscode.commands.registerCommand(
            'codeXR.codeAnalysis.fileClicked',
            (filePath: string) => {
                console.log('[CODE_ANALYSIS] Command: fileClicked executed');
                CodeAnalysisInteractionHandler.handleFileClick(filePath, context);
            }
        );
        
        // Theme toggle command
        const toggleThemeCommand = vscode.commands.registerCommand(
            'codexr.analysis.toggleTheme',
            async () => {
                console.log('[CODE_ANALYSIS] Command: toggleTheme executed');
                try {
                    const newTheme = await AnalysisSettingsStorage.toggleTheme(context);
        ...
        const setLightThemeCommand = vscode.commands.registerCommand(
            'codexr.analysis.setLightTheme',
            async () => {
                console.log('[CODE_ANALYSIS] Command: setLightTheme executed');
                try {
                    await AnalysisSettingsStorage.setTheme(context, 'light');
        ...
        const setDarkThemeCommand = vscode.commands.registerCommand(
            'codexr.analysis.setDarkTheme',
            async () => {
                console.log('[CODE_ANALYSIS] Command: setDarkTheme executed');
                try {
                    await AnalysisSettingsStorage.setTheme(context, 'dark');
        ...
        const setAutoAnalysisDelayCommand = vscode.commands.registerCommand(
            'codexr.analysis.setAutoAnalysisDelay',
            async () => {
                console.log('[CODE_ANALYSIS] Command: setAutoAnalysisDelay executed');
                try {
                    await CodeAnalysisInteractionHandler.handleAutoAnalysisDelaySelection(context);
        ...
        const selectChartTypeFileCommand = vscode.commands.registerCommand(
            'codexr.analysis.selectChartTypeFile',
            async () => {
        ...
        const mapDimensionFileCommand = vscode.commands.registerCommand(
            'codexr.analysis.mapDimensionFile',
            async (dimensionName: string, dataType: string, required: boolean) => {
            ...
            async () => {
                ...
                try {
                    ...
                    const result = await vscode.window.showWarningMessage(
                        'Reset all analysis settings to default values? This will restore chart type to "boats" and clear all dimension mappings.',
                        { modal: true },
                        'Reset Settings',
                        'Cancel'
                    );
                    ...
                    if (result === 'Reset Settings') {
                        ...
                        await AnalysisSettingsStorage.resetToDefaults(context);
        ...
        context.subscriptions.push(
            showActiveAnalysesCommand,
            showAnalysisSettingsCommand,
            showFilesByLanguageCommand,
            placeholderActiveAnalysesCommand,
            placeholderAnalysisSettingsCommand,
            placeholderFilesByLanguageCommand,
            toggleAnalysisModeCommand,
            analyzeFileStaticCommand,
            analyzeFileXRCommand,
            refreshCommand,
            fileClickedCommand,
            toggleThemeCommand,
            setLightThemeCommand,
            setDarkThemeCommand,
            setAutoAnalysisDelayCommand,
            selectChartTypeFileCommand,
            mapDimensionFileCommand,
 ...
 */
export async function executeFileAnalysis(context: vscode.ExtensionContext, filePath: string): Promise<any> {
 ...
 */
async function runStaticFileAnalysisCoordinator(context: vscode.ExtensionContext, filePath: string): Promise<any> {
    return new Promise((resolve, reject) => {
        ...
        const pythonEnvStorage = new PythonEnvStorage(context);
        const venvPath = pythonEnvStorage.getVenvPath();
        const pythonExecutable = PythonEnvUtils.getVenvPythonPath(venvPath);
        ...
        const pythonProcess = spawn(pythonCommand, [coordinatorPath, filePath], {
 ...
 */
export async function runXRFileAnalysisCoordinator(context: vscode.ExtensionContext, filePath: string): Promise<any> {
    return new Promise((resolve, reject) => {
 ...
 */
async function handleChartTypeFileSelection(context: vscode.ExtensionContext): Promise<void> {
    ...
    try {
        ...
        const quickPickItems: (vscode.QuickPickItem & { chartId: string })[] = availableCharts.map(chart => ({
            label: chart.name,
        ...
        if (selectedItem && selectedItem.chartId) {
            ...
            await AnalysisSettingsStorage.setChartTypeFile(context, selectedItem.chartId);
 ...
 */
async function handleDimensionMappingFileSelection(
    ...
    try {
        ...
        if (dataType === 'numeric') {
            availableOptions = fieldOptions.filter(option => 
                option.dataType === 'numeric'
            );
        }

        // Create quick pick items
        const quickPickItems: vscode.QuickPickItem[] = availableOptions.map(option => ({
            label: option.label,
        ...
        if (selectedField && selectedField.label) {
            ...
            await AnalysisSettingsStorage.updateDimensionMappingFile(context, dimensionName, selectedField.label);
