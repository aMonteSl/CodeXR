...
import { HashBasedChangeDetector, FileWithHash } from '../shared/hashBasedChangeDetector';

/**
 * Shared directory watcher utility for all directory analysis types
 */
export interface DirectoryWatcherOptions {
  ...
  onAnalysisUpdate?: (result: DirectoryAnalysisResult) => Promise<void>;
  
  /** Callback when analysis starts */
  onAnalysisStart?: (directoryPath: string) => void;
  
  /** Callback when analysis completes */
  onAnalysisComplete?: (result: DirectoryAnalysisResult) => void;
...
export class SharedDirectoryWatcher {
  ...
  private lastAnalysisResult: DirectoryAnalysisResult | undefined;
  private debouncedAnalysisFunction: (() => void) | undefined;
  ...
  constructor(directoryPath: string, options: DirectoryWatcherOptions) {
  ...
  private loadConfiguration(): void {
    ...
    const newDebounceDelay = config.get<number>('codexr.analysis.debounceDelay', 2000);
   ...
   */
  private createDebouncedAnalysisFunction(): void {
    ...
    this.debouncedAnalysisFunction = debounce(async () => {
  ...
  private async performIncrementalAnalysis(): Promise<void> {
    ...
    this.statusMessage = vscode.window.setStatusBarMessage(
      `$(sync~spin) CodeXR: Detecting changes in ${dirName}/...`
    );
    ...
    const result = await vscode.window.withProgress({
      ...
      const currentFiles = await HashBasedChangeDetector.getCurrentFileListWithHashes(
        this.directoryPath,
        this.options.includeSubdirectories || false
      );
      
      // Convert previous analysis result to FileWithHash format if available
      const previousFiles = this.lastAnalysisResult 
        ? HashBasedChangeDetector.convertFromFileMetrics(this.lastAnalysisResult.files)
        : [];
      
      // Perform hash-based change detection
      const changeDetection = HashBasedChangeDetector.detectChanges(
        this.directoryPath,
        currentFiles,
        previousFiles
      );
      ...
      const analysisManager = new DirectoryAnalysisManager();
      
      // Use appropriate filters based on mode and subdirectory inclusion
      const filters = this.options.includeSubdirectories 
        ? { ...DEFAULT_SHALLOW_FILTERS, maxDepth: 10 } // Deep analysis
        : { ...DEFAULT_SHALLOW_FILTERS, maxDepth: 1 };  // Shallow analysis
      ...
      const fullAnalysisResult = await analysisManager.analyzeDirectory(
        this.directoryPath,
        filters,
        this.lastAnalysisResult // Pass previous result for incremental analysis
      );
   ...
   */
  startWatching(initialResult?: DirectoryAnalysisResult): void {
  ...
  private showErrorMessage(error: unknown): void {
    const dirName = path.basename(this.directoryPath);
    this.statusMessage = vscode.window.setStatusBarMessage(
      `$(error) CodeXR: ${dirName}/ re-analysis failed`
    );
   ...
   */
  getLastAnalysisResult(): DirectoryAnalysisResult | undefined {
    return this.lastAnalysisResult;
  }
  ...
  private removeDeletedFiles(
    ...
    const updatedFiles = previousResult.files.filter(
      file => !deletedSet.has(file.relativePath)
    );
  ...
  private mergeAnalysisResults(
    previousResult: DirectoryAnalysisResult,
    newResult: DirectoryAnalysisResult,
    changeDetection: ReturnType<typeof HashBasedChangeDetector.detectChanges>
  ): DirectoryAnalysisResult {
   ...
   */
  private calculateSummary(directoryPath: string, files: any[]): any {
 ...
 */
export class SharedDirectoryWatcherManager {
  private static instance: SharedDirectoryWatcherManager | undefined;
  private watchers: Map<string, SharedDirectoryWatcher> = new Map();
  private initialized: boolean = false;
  
  /**
   * Gets the singleton instance
   */
  static getInstance(): SharedDirectoryWatcherManager {
   ...
   */
  private updateAllWatcherConfigurations(): void {
   ...
   */
  getLastAnalysisResult(directoryPath: string): DirectoryAnalysisResult | undefined {
    const watcher = this.watchers.get(directoryPath);
