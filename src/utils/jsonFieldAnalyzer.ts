...
export interface JsonFieldInfo {
    ...
    /** Number of non-null values found */
    valueCount: number;
}
 ...
 */
export class JsonFieldAnalyzer {
    
    /**
     * Analyze a JSON file and extract field information
     */
    public static async analyzeJsonFile(filePath: string): Promise<JsonAnalysisResult> {
    ...
    private static analyzeDataStructure(data: any, filePath: string): JsonAnalysisResult {
        const fields = new Map<string, JsonFieldInfo>();
        let recordCount = 0;
     ...
     */
    private static getValueType(value: any): JsonFieldInfo['type'] {
     ...
     */
    public static formatFieldForDisplay(field: JsonFieldInfo): { label: string; description: string; detail: string } {
