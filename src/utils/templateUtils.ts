 ...
 */
export async function processTemplate(context: vscode.ExtensionContext, chartSpec: ChartSpecification): 
    Promise<{ html: string, originalDataPath?: string, isRemoteData?: boolean }> {
 ...
 */
export async function saveHtmlToFile(content: string, defaultName: string, originalDataPath?: string, isRemoteData?: boolean): Promise<string | undefined> {
  const workspaceFolders = vscode.workspace.workspaceFolders;
  ...
  try {
    ...
    if (originalDataPath) {
      ...
      if (isRemoteData) {
        ...
        let localJsonPath: string | undefined;
