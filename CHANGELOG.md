...
- **You stand at your own height in VR and AR.** The eye-height offset used to sit on the camera rig, where WebXR's `local-floor` reference space — which already carries your real height — stacked on top of it, leaving you hovering almost two metres above the virtual room with the pedestal around your shins. The offset now lives on the camera itself, which is where A-Frame replaces it with the headset's own pose and restores it when you leave. Side effect worth having: your avatar's feet now reach the floor for everyone else in the room.
