...
- **AR brings the pedestal to you.** Entering AR hides the virtual room and environment (that part already worked) and now also recenters you: the pedestal, its controller and the screens appear right in front of you, on your own floor, instead of seven virtual metres away — possibly behind a real wall. Exiting restores your exact previous position. VR is untouched: the full room stays visible.
...
- **You stand on the floor in VR and AR, not 1.75 m above it.** The camera rig carries a desktop eye-height offset, but WebXR's `local-floor` reference space already adds your real height — so in a headset the two stacked and left you hovering almost two metres over the virtual room, with the pedestal somewhere around your shins. On entering any immersive session the rig now drops to floor level, and your desktop position is restored when you exit.
