...
- **Bubbles are spheres again, with sizes you can actually compare.** A chart switch used to inherit the previous chart's stretched scale (bubbles rendered as vertical lozenges), and the default radius metric was so skewed that every typical file was an invisible speck — the default mappings now prefer average complexity for radius and language for the depth axis, which also declutters the map charts.
