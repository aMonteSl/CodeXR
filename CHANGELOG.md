...
**Your headset does not need any of this.** A VR session over your own network is the complete experience: the analysis server already speaks HTTPS for WebXR, and a headset on the same Wi-Fi opens the scene directly. Cross-network exists for the other case, someone who is *not* on your network. It comes **enabled by default** (each server opens its tunnel as it starts) and can be turned off in Server Configuration.

- **How you connect someone.** The tunnel opens with the server (or press *Start remote access* if you had stopped it). `cloudflared` opens an **outbound** tunnel to Cloudflare (your router never has to accept a new inbound connection, and CodeXR does not publish your IP), and the invitation link lands on your clipboard. Your guest opens it, picks the name they will appear with, and CodeXR shows **you**, in VS Code, a temporary six-digit code with a *Copy code* button. You pass that code to them however you already talk; they type it and they are in.
- **The code is the gate.** It expires, it allows a limited number of attempts, and a wrong one **burns itself immediately** and warns you, with a one-click action to issue a replacement. Behind it: one-use browser tokens, an `HttpOnly` session cookie, per-address rate limits, and complete revocation of invitations, sessions and credentials the moment you stop sharing (the random URL simply ceases to exist). The invitation link never ages out: it works for as long as you keep sharing and dies the moment you stop. `cloudflared` itself is pinned to 2026.5.2, downloaded only with your consent (asked once and remembered) and verified by SHA-256 before it is ever executed.
...
- **Preview AR and VR without a headset.** `CodeXRDebug.simulateAR()`, `simulateVR()` and `exitSimulated()` in any generated scene's browser console show what each immersive mode hides, keeps and where it places you. They are not a WebXR session (no headset pose, stereo or passthrough) and say so; see `docs/xr-testing/XR_DEBUG_COMMANDS.md`. For the full experience, emulated controllers with working thumbsticks and triggers included, there is a step-by-step tutorial for Meta's Immersive Web Emulator in `docs/xr-testing/WEBXR_EMULATOR_TUTORIAL.md`.
...
- **The cross-network address no longer dies after 30 minutes.** Invitation links carried a 30-minute TTL while the tunnel kept running and the sidebar kept showing the address, so a guest opening it half an hour in landed on a 404. An invitation now lives exactly as long as the share does: it works until you stop sharing, and revocation still kills it instantly. The real gate against a leaked link was never the timer but the six-digit code, which keeps its 5-minute window, its attempt limit and its burn-on-first-mistake behavior.
- **Cross-network stays honest about its own state.** Turning it off in Server Configuration now also removes the `cloudflared` copy CodeXR downloaded for it (a system-wide install on your PATH is never touched, since that one is not CodeXR's to remove). And declining the one-time download prompt no longer leaves the setting sitting on a silently broken "Enabled": it flips to Disabled, so the row tells the truth, and re-enabling it offers the download again instead of staying stuck.
...
- **The sticks work in the WebXR emulator too.** Meta's Immersive Web Emulator reports its controllers' gamepads as *not connected* even while they stream input (its runtime only syncs that flag inside a setter nothing invokes on session start), and aframe-extras gates all stick locomotion on exactly that flag, so triggers and lasers worked while both sticks were silently dead. CodeXR now also trusts the session's own input-source list, which the WebXR Gamepads Module declares connected by definition; real headsets are unaffected and the emulator behaves like one.
- **The laser survives changing hands.** Handing the pointer to the other controller used to recreate its cursor, and creating a cursor makes A-Frame reset the ray to the bare controller axis, silently undoing the ~40°-down compensation that maps a Touch controller's grip onto where it actually points. From then on that hand aimed high again. The inactive hand now keeps its (inert) cursor, so the pointing direction installed from the controller model is never wiped.
- **Leaving VR puts you back at your desktop spot.** Flying in VR could leave you mid-air or outside the room, and exiting only restored your height, stranding you on the environment floor with no way back (desktop mode cannot fly or cross walls). Exiting any immersive session now restores the exact position and heading you had before entering, the same way AR always did.
- **Grab a screen in VR or AR and push or pull it with the stick.** With a mouse you drag a virtual screen and scroll to bring it closer or push it away; the controller equivalent existed but never worked. It applied a step only when the stick's value *changed* (holding it deflected did nothing), its direction was inverted, and the stick simultaneously kept walking or turning you. Now the grabbing hand's thumbstick moves the grabbed screen smoothly for as long as you hold it: forward pushes away, back pulls closer (capped before it reaches your face), left and right slide it sideways, and diagonals combine both. That stick stops driving locomotion for the duration of the grab (the other hand keeps walking and turning), and no amount of activity on the other controller can snatch the laser mid-grab.
- **The second laser that would not turn off.** Switching hands while the old laser was pointing at something could leave its beam permanently frozen in the scene: A-Frame's raycaster schedules its line redraw with a timer and redraws without re-checking whether the line should still be visible, so the redraw queued just before the handover resurrected the beam after CodeXR had hidden it. Demoted lasers are now swept clean one beat later.
- **No more white ring floating in front of your eyes.** The gaze cursor's reticle showed up in every simulated immersive entry and flashed at headset entry. The reticle geometry is gone everywhere (gaze interaction still works through its raycaster on the devices that genuinely need it: mobile AR, sleeping controllers), and simulated entries now sensibly keep the **mouse** as the pointer: you are still at a desk.
- **AR really removes the room.** Hiding the walls and floor was only visual: their invisible colliders kept truncating the laser mid-air and stealing hover, and screens still bumped into walls nobody could see. In AR the room's pieces now also stop being ray targets and the screen bumpers ignore the room shell entirely, so screens move freely in your own space, while VR and desktop keep colliding with the room exactly as before.
- **AR is no longer dim.** The environment's lights are parented under the environment itself, which AR hides, so charts, pedestal and logo dropped to flat ambient lighting the moment you entered AR. A dedicated directional fill light now comes on for AR sessions only (and off on exit), restoring depth and volume without changing how desktop or VR look.
...
- A dedicated immersive-semantics harness (`test:xr-immersive-harness`) runs the AR/VR contract against real A-Frame in Chromium: what AR hides (environment + room) and keeps (pedestal, charts, panel, screens), the rig floor-alignment and AR recenter, and the mouse → gaze → laser pointer handover, plus a static contract pinning both scene templates. Its fake controllers now mirror what the Immersive Web Emulator's runtime actually exposes (`connected: false` gamepads, `[null, null, x, y]` axes, and the real `controllermodelready` ray payload), so the emulator-only failure modes fail in CI instead of only in a live headset session.
- The VR/AR experience was driven end to end inside a real Chromium with the Immersive Web Emulator, remotely over the `@iwer/extension-bridge` MCP daemon: session entry/exit for both modes, stick locomotion and flight, trigger clicks on live Babia charts, and screenshot-verified height and AR recentering. That is what surfaced the three fixes above.
...
- **Debugged in emulated XR before shipping**: Meta's [Immersive Web Emulator](https://github.com/meta-quest/immersive-web-emulator) made it possible to drive real WebXR sessions from a desktop browser and debug the immersive experience end to end: entry height and positioning, what AR hides and keeps (environment, room, invisible colliders), stick locomotion and flight, laser aim and hand switching, AR lighting, and grabbing screens with the controller all got fixed this way. `docs/xr-testing/WEBXR_EMULATOR_TUTORIAL.md` explains how to reproduce that setup, and `docs/xr-testing/XR_MANUAL_CHECKLIST.md` is the manual validation script.
...
- **The exported copy opens in the mode you left active.** A movie snapshot arriving at scene boot used to steal the table: the project-evolution runtime activated its mode unconditionally whenever its shared entity carried a result, so an exported copy (and, latently, a guest replaying a room snapshot) could land on the evolution movie even when the classic analysis was the last thing on screen. Mode-data entities are now passive by contract (they store their payload and render only if their mode is already active), and the export manifest carries the authoritative `analysis-view` entity with the mode active at export time, as an opaque string, so future modes inherit the behavior without touching this code.
- **Choose what you export, and take the whole git history with you.** Export Analysis Folder now opens a multi-select modal listing the four analyses. Ticking the git analyses pre-analyzes the entire git timeline (up to 240 revisions, deduplicated by commit and by content, four analyses in parallel, cancellable with an honest partial result) into per-revision payloads shared by both modes. In the exported copy the controllers then work for real: the historical picker lists every exported revision and Compare renders any pair instantly with locally computed change counters, and the evolution panel generates Auto, Range or Manual movies on the spot using the same time-spread sampler the live server uses. Exports made without git data keep the previous replay behavior.
- **Self-contained exports**: Export Analysis Folder now produces a copy that works on any static HTTP server, without CodeXR behind it. The export writes a manifest that stands in for the live collaboration session, rewrites the artifact URLs so sub-path serving works, refreshes the runtime copies, and drops a `README-EXPORT.md` with instructions. The classic analysis works fully; the dependency graph works too, and if it was never opened the export runs the analysis first (a progress notification appears); historical comparisons and the evolution movie replay everything computed before export, with clear read-only notices where recomputing would need the live session. The exported scene opens no WebSocket and no SSE channel, so its console stays clean.
