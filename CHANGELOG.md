...
- **There is no viewer limit.** The browser sharing a screen encodes it **once** and uploads a single copy no matter how many people are watching; extra viewers only ask for a keyframe to join in. What does grow is the fan-out, since every remote viewer has their own connection through the tunnel, so quality follows the audience automatically (1.5 Mbps for one or two people, down to 350 kbps for a crowd) — the same encoder is retuned, never duplicated. The host's screen shows how many people are watching and roughly what it is costing their upload.
- If one viewer's connection struggles, **only that viewer degrades**: the video carries three temporal layers, so the server thins their stream to a lower frame rate while everyone else keeps the full picture. Keyframes and audio are never dropped, so they recover on their own. See `docs/CLOUDFLARE_REMOTE_ACCESS.md` for the bandwidth table.
...
- **New `Meet the Creator` entry** in LEARN MORE & SUPPORT, opening `https://adrianmonteslinares.com/`. The README gains an *About the Author* section and an author badge alongside the existing ones.

### Added — `About BabiaXR` credits in the examples view

- **BABIA EXAMPLES now ends with an `About BabiaXR` row** that opens a dialog crediting the libraries the example scenes are built on — BabiaXR (GPL-3.0), A-Frame, aframe-environment-component and aframe-extras (MIT) — with buttons to open the BabiaXR site, its source and its licence. Same native-dialog treatment as `3D Model` and the server dialogs, sharing one formatter. The row stays reachable even when no examples are found.
- **Fixed an attribution gap**: the bundled example scenes load four third-party libraries from CDNs, but `THIRD_PARTY_NOTICES.md` documented only two. `aframe-environment-component` and `aframe-extras` are now credited, and the A-Frame/BabiaXR entries record that the examples load them too — at different versions from the XR templates (A-Frame 1.0.4 vs 1.7.1; BabiaXR unpinned vs 1.3.4), which the dialog reports honestly.
...
### Changed — Every player gets their own colour, a name tag that follows them, and a smaller avatar

- **Each participant now reads as a distinct person.** Previously everyone ended up the same colour: `avatarId` collapsed missing/invalid values to `avatar-1`, so "picked Azure" and "never picked" were indistinguishable. There is now an **`Automatic`** option (the new default) meaning *let the room choose*, and **the room assigns a colour nobody else there is wearing** — the same way it already hands out unique Star Wars aliases. An explicitly chosen colour always wins, live profile changes still propagate, and switching back to Automatic re-allocates a free colour. Resolution happens server-side, so every client (and the `Connected users` list) sees the same colour for a given player.
  - Existing profiles storing `avatar-1` are migrated to `Automatic` once, since that value was the old default rather than a deliberate pick. Any other colour is left untouched.
- **The colour actually shows on the model.** The tint was a 22 % blend, barely visible on the robot's own materials; it is now a tunable `tintStrength` at 55 %, so the player colour dominates while the model keeps its shading.
- **Name tags sit above each head and turn to face you.** Every viewer sees every other player's name head-on, wherever those players are looking — the tag yaws toward the local camera (yaw only, so text never tilts when you crouch or fly) and cancels out the avatar's own rotation. Names get a subtle dark backing plate so they stay legible against light charts.
- **The avatar is a little smaller** — fitted to 1.55 m instead of 1.70 m, which suits the robot's oversized head without losing the human scale reference next to the visualizations.

### Changed — One `Name` row, and a better avatar model that fits itself to human height

- **`Identity` and `Name` were two rows describing the same thing** ("Custom name" / "Star Wars alias assigned on connect"). They are now a single **`Name`** row showing what you are actually called — your custom name, or `Anonymous`. Picking "Custom name" always opens the prompt pre-filled, so the one row both switches mode and edits the name.
- **The downloadable avatar is now Robot Expressive** (Tomás Laulhé, modified by Don McCurdy) instead of the Quaternius base character:
  - **CC0 1.0** (public domain) rather than CC BY 3.0 — no attribution obligation for users of generated scenes.
  - **0.44 MiB instead of 2.16 MiB** (5× smaller) and 3,237 triangles instead of 13,744 — a lighter download and a lighter scene on headsets.
  - Its clips are literally `Idle` / `Walking` / `Running`, so the runtime's animation matcher hits them by name instead of falling back to clip indices.
  - The URL is pinned to the commit that introduced the file (unchanged since 2018) and **the download is now verified against a recorded SHA-256 digest** — the avatar previously had no integrity check at all, unlike `cloudflared`. A mismatch is discarded rather than installed.
- **The avatar runtime now fits any model to a human height** instead of assuming the asset is authored in metres. The new model is authored in centimetres (~4.6 units tall) and would have rendered as a giant; the runtime measures the loaded model, scales it to a configurable `avatarHeight` (1.7 m) and drops its feet on the floor. Swapping the model again needs no new magic number.
- The avatar download consent dialog was still Spanish; it now reads in English and names the model, author and license.

### Changed — The guest pairing page now wears the Code-XR identity

- **The `/join` page a remote guest opens has been restyled** after the product site (code-xr.adrianmonteslinares.com): black canvas with a soft blue halo and two blurred colour orbs, a glass card (`rgba(255,255,255,.05)` over `blur(12px)`), the Code-XR mark beside a white→neon-blue gradient wordmark, identity options as selectable rows that light up in `#00aaff`, and a neon primary button with the site's glow. The six-digit field is now a wide, letter-spaced monospace input — the one place where a distinct treatment actually helps. **No behaviour changed**: same two-step flow, same endpoints, same messages.
- **An expired or already-used invitation no longer dumps raw JSON** (`{"error":true,"status":404,…}`) into the guest's browser. It now renders a styled page explaining that invitation links expire after 30 minutes and only work while the host is sharing, so a stale link lands somewhere self-explanatory. API responses are untouched — only this HTML-facing branch changed.
- Markup, styles and page script moved out of `httpServer.ts` into `src/servers/runtime/remote/pairingPage.ts`, with the brand mark inlined as SVG in `brandMark.ts`. Everything stays self-contained by necessity: the response CSP sends no `img-src` (so `data:` images are blocked) and an unpaired guest is not authorised to fetch static assets, so external stylesheets, web fonts and images would all fail. A new test pins that invariant.

### Fixed — Servers really stop (and leave the list), and every server dialog is readable

- **A stopped server no longer lingers in ACTIVE SERVERS as `(stopped)`.** The cause was a shutdown deadlock, not a display bug: `stop()` awaited `server.close()`, which waits for every socket to drain — while the sockets that never drain on their own (collaboration/broadcast WebSockets, open SSE responses) were released by `disposeRuntimeFeatures()`, which ran *inside* the close callback. The promise never settled, so the registry entry was never removed. Runtime features are now disposed and connections dropped **before** awaiting the close, in all three server classes. Measured against a live SSE response plus a WebSocket peer: `close()` alone still hangs; the new order completes.
- **The row leaves the list no matter what.** `ServerControl.stopServer` now races the graceful shutdown against a 4-second timeout, attempts a forced socket close (`forceStop()`) if it expires or fails, and unregisters the server in a `finally` — so no runtime failure can leave a zombie row. An unclean shutdown says so: *"Server removed from the list, but its runtime did not shut down cleanly: …"*. The tree additionally renders only running servers, so a hang can never surface a stale row again.
- **Fixed: the Server Information dialog printed literal `\n`** and ran every field onto one line. All three server dialogs — Server Information, Cross-network connection status, and participant details — are now built by one formatter: aligned label columns, blank lines between groups, real newlines, and humanized values (`HTTPS (self-signed)` instead of `HTTPS-DEFAULT`, `Running · up 21s`, `1 request waiting` instead of `1 pending request(s)`, long storage paths elided to their tail). Server Information also shows the LAN address and, while shared, the cross-network block; dialog titles now name the server rather than repeating its URL.
- **A single server now opens expanded**; with two or more they stay compact. The count lives in the ACTIVE SERVERS header label, so changing it re-keys the subtree and the new state applies in both directions — stopping one of two re-expands the survivor.

### Changed — A wrong pairing code burns itself, warns the host, and can be replaced in one click

- **A failed pairing attempt now reaches the host.** Previously a wrong code threw silently at the guest: the host saw nothing, and the same code stayed usable for four more guesses. Now the first wrong guess raises a warning — `Failed pairing attempt from <name> (<ip>). That code is no longer valid.` — with a **`Generate new code`** button that mints a replacement and shows it right away.
- **The code dies with the failed attempt.** A burnt code is rejected even if the correct digits arrive afterwards, so brute-forcing a six-digit code drops from five guesses to one. Repeated submissions against an already-burnt code do not re-notify the host, so a guest hammering the form cannot spam notifications.
- **The connection request survives the failure**, so the guest stays in the queue: the host hands over a fresh code and the guest just types it — no re-pasting the invitation link. The CodeXR guest keeps its prompt open with the reason shown (`This code is no longer valid. Ask the host for a new one.`) instead of unwinding the whole join flow, and the browser guest now shows the server's real message rather than one blended "wrong/expired/out of attempts" string.
- **New host action `Generate new pairing code`**, inside `Cross-network connection` while somebody is waiting (`N waiting`). It also rescues the case where the code notification was dismissed before it could be read — previously unrecoverable, since only a salted hash is stored. With several people waiting it asks which one, showing who is still waiting for a code and whose code was invalidated.
- Codes are regenerated with the same CSPRNG path as the original (`crypto.randomInt` + fresh salt), reset the attempt budget, and refresh the 5-minute expiry. Regeneration is host-initiated only.
- The browser pairing page is now in English, matching the rest of the UI.
- `npm test` 377 (the authority suite now pins the burn/regenerate contract); verified end to end against a real authority: burn → single warning → regenerate → the new code pairs and the old one never does.

### Changed — Enabling cross-network connections now actually shares; sharing URL and participant details at hand

- **Enabling `Cross-network connections` starts sharing by itself.** Every running server begins publishing its tunnel the moment the setting is enabled, and every server launched afterwards auto-shares on start — before, the setting only flipped a flag and each server had to be shared by hand. Each auto-share announces itself with a toast carrying a `Copy invitation link` button (no silent clipboard overwrite). Stopping a specific server's tunnel from its `Cross-network connection` group works as before and stays stopped — auto-start reacts to server launches, never to state changes.
- **The cloudflared consent still gates everything**: on a cold install the download modal appears once; declining it suppresses further auto-start prompts for the session (re-enabling the setting or manually starting a connection offers it again), and no server state is touched by a decline.
- **The sharing URL is now a first-class row item**: while shared, `Cross-network address` appears right below `Local network address` with the invitation URL visible — one click copies it. The `Cross-network connection` group is pure control now (`View remote status` / `Stop remote connection` / `Start remote connection`).
- **Clicking a connected user opens a details modal**: name, role (Host/Guest), avatar, client, connection scope, **IP address**, and connected-since. The room server now records each peer's address at WebSocket connect (`cf-connecting-ip` first, so tunneled guests show their real IP instead of local cloudflared) — it was previously discarded after pairing. `ConnectedParticipantSummary` gains `role` and `remoteAddress` (additive).
- **Fixed: the `Connected users` group had no icon** — it referenced `accounts`, which is not a codicon and silently rendered nothing; now `organization`.
- Verified against a real `CollaborationRoomServer` over WebSocket (summary carries role + IP) and the mock-vscode tree harness (flat address item, control-only group, participant command, auto-start gating incl. decline suppression); `npm test` 376 green.

### Changed — Server rows organized around the cross-network setting; the setting itself now in English

- **Every remote-access control on a server row now lives in one `Cross-network connection` group** — and the group only exists when the global setting allows it. Before, a flat `Remote connection` status line, a conditional `Copy remote invitation` line, and remote actions mixed into `Actions` were always shown, and `Start remote connection` simply errored when the setting was disabled.
- **Setting disabled** (the default): a server row is purely local — `Local network address`, `Connected users`, and an `Actions` group with only local actions (`Open in browser`, `Open in panel` for HTTP, `Copy address`, `Server information`, `Stop server`).
- **Setting enabled**: the `Cross-network connection` group appears after the local address, carrying the tunnel status as its description (`Not shared` / `Starting…` / `Shared | N pending request(s)` / the error) and status-appropriate children: `Start remote connection` when stopped; `Invitation link` (click copies) + `View remote status` + `Stop remote connection` while shared; `View remote error` + a retry when failed. `Actions` is identical in both modes. Defensive rule: a live tunnel is never hidden — if the setting is disabled underneath a shared server, the group stays until the tunnel stops.
- **The left-click QuickPick mirrors the tree**: `getAvailableActions` now consults the setting, so `Start remote connection` is no longer offered (only to fail) when cross-network connections are disabled.
- **The SERVERS setting speaks English**: `Cross-network connections: Enabled|Disabled` (label, tooltip, and the toggle QuickPick), including the label-substring click routing in `ServersSectionProvider` and the error message in `RemoteAccessManager` that quotes it. Command IDs unchanged.
- Verified with the mock-vscode harness over the real tree provider across six states (disabled, enabled+stopped, enabled+shared, enabled+error, HTTP variant, disabled-while-shared) plus the QuickPick in each; `npm test` 376 green with the updated source assertion.

### Added — `Reset to Default` in Visualization Settings

- **A single action restores the four visualization settings at once** — background color, ground color, environment preset and chart palette. It sits at the bottom of `VISUALIZATION SETTINGS`, under the fields it resets, and runs the new `codeXR.visualizationSettings.resetAll` command on top of the storage layer's existing `resetSettings()`.
- **It says what it is about to change before doing it**: the confirmation is a modal listing each field as `current → default` (e.g. `Chart Palette: sunset → ubuntu`), so nothing is lost by a stray click; cancelling writes nothing. If every setting already holds its default, it reports that instead of prompting.
- The default-color swatch icons are regenerated as part of the reset (and the stale ones cleaned up), so the tree shows white/black immediately rather than the previous custom colors.

### Changed — Collaboration now lives inside Active Servers, and the whole section speaks English

- **`COLLABORATION` is no longer a top-level sidebar section.** Its five entries (`Join Remote Session`, `Identity`, `Name`, `Avatar Color`, `3D Model`) now hang from an expanded `COLLABORATION` group that is the first child of `ACTIVE SERVERS` — the servers you host and the session you join finally sit in one place. The root sections drop from nine to eight.
- **One implementation, not a copy.** The group mounts the existing `CollaborationSectionProvider` and adapts its items into the Active Servers tree, so the profile items and `codeXR.collaboration.joinRemote` keep the exact behaviour they had; nothing about the invitation-link → 6-digit-code pairing flow changed. The group renders even with **zero servers running** (next to `No active servers`), because joining a remote session never required a local server. Profile edits still repaint the group: the section bridges the collaboration provider's change event, and the collaboration commands now refresh `activeServers`.
- **The whole section is in English.** The tree labels, descriptions and tooltips, the identity/name/avatar/3D-model quick picks and input boxes, the server-row items (`Local network address`, `Remote connection`, `Connected users`, `Actions`), every server action, the remote-status modal, and all `remote_access` messages (pairing dialogs, tunnel errors, the cloudflared download consent prompt) were Spanish and now read in English. Command IDs, `contextValue`s and the `package.json` contributions are untouched.
- Verified by walking the real `ModularTreeDataProvider` path against a stubbed VS Code API: root sections no longer include COLLABORATION, the group and its five leaves resolve with their commands through the modular item round-trip, a profile change re-renders the group, and a populated section (two servers, one shared over a tunnel) still produces every server child, the conditional invitation item and both remote action sets — all in English. `npm test` 376 green.
