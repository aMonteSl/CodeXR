...
- **Universal XR Chart Pedestal and Table Layout**: XR charts now render on a shared CodeXR chart pedestal/table that recenters the chart, keeps it inside a useful size band, and stabilizes the visualization after rebuilds or remaps without requiring chart-specific layout rules.
...
- **Safe Mapping Recovery Inside XR**: XR chart remapping now applies selections tentatively, lets Babia rebuild the chart, and automatically restores the last valid mapping if the resulting geometry becomes invalid. Failing field/axis combinations are temporarily disabled for the session so the immersive scene stays stable while the user tries another mapping.
