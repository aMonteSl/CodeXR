...
- **Preview AR and VR without a headset.** `CodeXRDebug.simulateAR()`, `simulateVR()` and `exitSimulated()` in any generated scene's browser console show what each immersive mode hides, keeps and where it places you. They are not a WebXR session — no headset pose, stereo or passthrough — and say so; see `docs/xr-testing/XR_DEBUG_COMMANDS.md`. For the full experience — emulated controllers with working thumbsticks and triggers included — there is a step-by-step Spanish tutorial for Meta's Immersive Web Emulator in `docs/xr-testing/WEBXR_EMULATOR_TUTORIAL.md`.
