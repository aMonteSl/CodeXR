...
- **You enter VR and AR at the same height you had in the browser.** Entering an immersive mode used to leave you either floating metres above the room or with your eyes at floor level, under the pedestal, depending on the device. The eye height now lives permanently on the camera rig — the one transform WebXR never rewrites — so the height you see in a normal browser tab is exactly the height you get when you put the headset on, in VR, in AR, and in the desktop simulate commands alike. And since flying is enabled in immersive modes, you can adjust freely from there.
