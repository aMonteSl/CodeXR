...
async function collectChartData(chartType, context) {
    try {
        ...
        while (!dimensionsSelected) {
            ...
            processedJsonPath = await (0, dataProcessor_1.createProcessedJsonFile)(dataSource, dimensions, context);
            // If processedJsonPath is undefined, the user canceled grouping
            if (!processedJsonPath) {
                const tryAgain = await vscode.window.showInformationMessage('Would you like to select different dimensions?', 'Yes', 'No');
                if (tryAgain !== 'Yes') {
...
async function collectDimensions(data, chartType) {
    try {
        ...
        vscode.window.showInformationMessage(`You'll select ${totalSteps} dimensions for your ${chartType}. You can go back to change previous selections.`);
        ...
        const minDimensions = chartType === chartModel_1.ChartType.CYLS_CHART ? 2 : dimensions.length;
        if (selectedDimensions.length < minDimensions) {
            vscode.window.showErrorMessage(`This chart type requires at least ${minDimensions} dimensions, but only ${selectedDimensions.length} were selected.`);
            return undefined;
        }
