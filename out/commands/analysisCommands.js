...
function registerAnalysisCommands(context, analysisViewProvider) {
    ...
    disposables.push(vscode.commands.registerCommand('codexr.analyzeFolder', async (folderUri) => {
        try {
            ...
            vscode.window.showErrorMessage(`Error analyzing folder: ${error instanceof Error ? error.message : String(error)}`);
        }
    }));
    ...
    disposables.push(vscode.commands.registerCommand('codexr.showJSMetricsDetails', () => {
        ...
        if (!editor)
            return;
        const fileAnalysis = context.globalState.get(`jsAnalysis:${editor.document.uri.fsPath}`);
...
function showDetailedMetrics(metrics) {
    vscode.window.showInformationMessage(`JavaScript Metrics:
    ...
    • Classes: ${metrics.classCount}`, { modal: true });
}
